...
function bsp_display_edit_profile_link ($atts) {
	...
	if (!is_user_logged_in())
		...
		if (empty($atts['edit'])) $atts['edit'] = '' ;
		if (empty($atts['redirect'])) $atts['redirect'] = '' ;
		if (!empty($atts['label'] ) ) {
			$edit_profile=$atts['label'] ;
			}
		...
		//get url
			$url = get_site_url(); 
		//if ($atts['redirect'] = 'y') 
			//wp_redirect( $url .'/' .$slug. $user .$edit.'">');

			$profilelink = '<a href="'. $url .'/' .$slug. $user .$edit.'">'.$edit_profile.'</a></li>';
	echo $profilelink ;
