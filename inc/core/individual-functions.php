...
// Post navigation.
function di_magazine_post_navigation() {
...
function di_magazine_posts_pagination() {
	if( get_theme_mod( 'display_archive_pagination', 'nextprev' ) == 'nextprev' ) { // Next-Pre pagination
		// Navigation.
		...
		?>
			...
			<nav class="navigation post-navigation dipostsnav" role="navigation">
				...
				<div class="nav-previous"><?php next_posts_link( ' &larr; ' . '<span class="oldr_entry_txt">' . esc_html( get_theme_mod( 'loop_oldr_enty_txt', __( 'Older News ', 'di-magazine' ) ) ) . '</span>' ); ?></div>

				<div class="nav-next"><?php previous_posts_link(  '<span class="nwr_entry_txt">' . esc_html( get_theme_mod( 'loop_nwr_enty_txt', __( 'Newer News', 'di-magazine' ) ) ) . '</span>' . ' &rarr; ' ); ?></div>

			</nav>
			<div class="clearfix"></div>
		<?php
		}
	} else { // Number pagination
		// Pagination.
		the_posts_pagination( array(
			'prev_text' => __( '&laquo;', 'di-magazine' ),
			'next_text' => __( '&raquo;', 'di-magazine' ),
		) );
	}
}


function di_magazine_breadcrumbs() {
	$custom_taxonomy = '';
	...
	if( !is_front_page() ) {
		...
		// Home page.
		echo '<li class="breadcrumb-item"><a href="' . esc_url( home_url( '/' ) ) . '">' . __( 'Home', 'di-magazine' ) . '</a></li>';

		if ( is_home() ) {

			echo '<li class="breadcrumb-item active">' . __( 'Blog', 'di-magazine' ) . '</li>';
			...
			if( !empty( $last_category ) ) {
				...
				// Category page.
				echo '<li class="breadcrumb-item active">' . __( 'Category: ', 'di-magazine' ) . single_cat_title( '', false ) . '</li>';
				...
				echo '<li class="breadcrumb-item active">' . __( 'Tag: ', 'di-magazine' ) . esc_html( $get_term_name ) . '</li>';
			...
			// Display author name.
			echo '<li class="breadcrumb-item active">' . __( 'Author: ', 'di-magazine' ) . get_the_author() . '</li>';

		} elseif( get_query_var( 'paged' ) ) {

			echo '<li class="breadcrumb-item active">' . __( 'Page: ', 'di-magazine' ) . get_query_var( 'paged' ) . '</li>';

		} elseif( is_search() ) {

			echo '<li class="breadcrumb-item active">' . __( 'Search: ', 'di-magazine' ) . get_search_query() . '</li>';

		} elseif( is_404() ) {

			echo '<li class="breadcrumb-item active">' . __( 'Error 404', 'di-magazine' ) . '</li>';

		} else {

			echo '<li class="breadcrumb-item active">' . __( 'Untitled', 'di-magazine' ) . '</li>';
...
function di_magazine_entry_meta() {
?>
	<span class="entry-meta">
		...
		<a href="<?php the_permalink(); ?>" ><span class="post-date updated" itemprop="dateModified"><?php if( get_theme_mod( 'post_date_view', '1' ) == 1 ) { echo the_date(); } else { echo the_modified_date(); } ?></span></a>
...
function di_magazine_post_thumbnail() {
...
// Menu Fallback.
function di_magazine_nav_fallback( $args ) {
	extract( $args );
	$output = null;
	if( $container ) {
		$output = '<' . $container;
	...
	$output .= '<li class="menu-item menu-item-type-post_type menu-item-object-page current-menu-item page_item current_page_item active"><a class="nav-link" href="' . esc_url( home_url( '/' ) ) . '">'. __( 'Home', 'di-magazine' ) .'</a></li>';
	
	if( current_user_can( 'manage_options' ) ) {
		$output .= '<li class="menu-item menu-item-type-custom"><a class="nav-link" href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">'. __( 'Add Menu', 'di-magazine' ) .'</a></li>';
	}
...
function di_magazine_comments( $comment, $args, $depth ) {
?>
		...
		<div class="comment-author vcard" itemtype="http://schema.org/Comment" itemscope itemprop="comment">
			<div id="comment-<?php comment_ID(); ?>" class="dimedia" >
				...
				<div class="dimedia-body">
					...
					<p class="alert alert-info" ><?php _e( 'Your comment is awaiting approval.', 'di-magazine' ); ?></p>
					<?php endif; ?>
							
					<small><?php //_e( 'On ', 'di-magazine' ); comment_date(); ?>
						<a itemprop="url" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>"> <?php printf( __( '%1$s at %2$s', 'di-magazine' ), '<span itemprop="datePublished">' . get_comment_date() . '</span>',  get_comment_time() ); ?></a>
					</small>
					...
					<?php comment_reply_link( array_merge( $args,
						array(
							...
							'reply_text' => __( 'Reply', 'di-magazine' ),
							)
					) );
					?>
					
					<?php edit_comment_link( __( 'Edit', 'di-magazine' ), '', '' ) ?>
					</small>
